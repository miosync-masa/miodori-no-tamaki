...
# beta_thermal(T) is the temperature-modified effective beta
# Using: A_inf(T) = 1 / (1 + K(T)*I)
# At reference conditions (T_opt, moderate I):
# beta_eff(T) / beta_eff(T_opt) = A_inf(T) / A_inf(T_opt)
...
# Panel D: beta_thermal relative to optimal
ax = axes[1, 1]
...
ax.set_xlabel('Temperature (C)')
ax.set_ylabel('beta_eff(T) / beta_eff(35C)')
ax.set_title('D: beta_thermal(T) closed form')
ax.set_ylim(0, 1.15)
ax.legend(fontsize=8)
ax.grid(alpha=0.3)
